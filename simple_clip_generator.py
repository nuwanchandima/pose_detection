...
MIN_CLIP_FRAMES = 30  # Minimum frames to save a clip (e.g., 30 frames = 1 second at 30fps)
...
def process_video(video_path: Path, min_clip_frames: int = MIN_CLIP_FRAMES) -> List[dict]:
    """
        ...
        if len(faces) == 1:
            ...
            if current_clip_start is None:
                ...
                if similarity < SIMILARITY_THRESHOLD:
                    # Face changed! Save previous clip if it's long enough
                    ...
                    if clip_length >= min_clip_frames:
                        ...
                        if success:
                            clips_info.append({
                                "clip_path": str(clip_path.relative_to(Path.cwd())),
                        ...
                        print(f"[Clip] Skipped short clip: {clip_length} frames (min: {min_clip_frames})")
...
async def upload_and_process(
    ...
    min_clip_frames: int = Form(default=30)
):
    ...
    try:
        ...
        # Process video
        clips_info = process_video(video_path, min_clip_frames)
